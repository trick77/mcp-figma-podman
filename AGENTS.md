...
- **No memory or cpu caps.** Rootless cgroup v2 hosts often lack `cpu`/`memory` controller delegation, so applying `MemoryMax`/`CPUQuota`/`mem_limit`/`cpus` makes the container fail to start. Only `TasksMax` (Quadlet) and `pids_limit` (compose) are set; they don't need to match (different units).
